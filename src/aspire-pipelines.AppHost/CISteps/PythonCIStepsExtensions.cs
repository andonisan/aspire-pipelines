            ...
            builder.Pipeline.AddStep($"{WellKnownCIStepNames.Setup}-uv-python", async stepContext =>
            {
                try
                {
                    ...
                    if (uvVersion is not null)
                    {
                        ...
                        {
                            stepContext.Logger.LogWarning("Already installed uv with version: {uvVersion}. Changing existing versions not supported yet",
                                uvVersionParts[1]);
                        }
                        else
                        {
                ...
                .WithPipelineStepFactory(factoryContext =>
                {
                    ...
                    return Task.FromResult(
                        lintingCommands.Select(lintCommand => new PipelineStep
                        {
                            ...
                            RequiredBySteps = [builder.LintingStepName],
                            DependsOnSteps = [builder.InstallStepName]
                        })
                    );
                });
        }
